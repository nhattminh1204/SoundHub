<%@ page pageEncoding="UTF-8" contentType="text/html; charset=UTF-8" %>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<footer class="text-center text-lg-start text-white">
    <!-- Grid container -->
    <div class="container p-4 pb-0">
        <!-- Section: Links -->
        <section>
            <!--Grid row-->
            <div class="row">
                <!-- Giới thiệu công ty -->
                <div class="col-md-3 col-lg-3 col-xl-3 mx-auto mt-3">
                    <h6 class="text-uppercase mb-4 font-weight-bold">
                        Sound Hub
                    </h6>
                    <p>
                        Sound Hub chuyên cung cấp tai nghe, loa và phụ kiện âm thanh chất lượng cao, 
                        mang đến trải nghiệm nghe nhạc tuyệt vời cho mọi khách hàng.
                    </p>
                </div>

                <hr class="w-100 clearfix d-md-none" />

                <!-- Sản phẩm -->
                <div class="col-md-2 col-lg-2 col-xl-2 mx-auto mt-3">
                    <h6 class="text-uppercase mb-4 font-weight-bold">Sản phẩm</h6>
                    <p><a class="text-white" href="#">Tai nghe True Wireless</a></p>
                    <p><a class="text-white" href="#">Tai nghe chụp tai</a></p>
                    <p><a class="text-white" href="#">Tai nghe có dây</a></p>
                    <p><a class="text-white" href="#">Loa Bluetooth</a></p>
                </div>

                <hr class="w-100 clearfix d-md-none" />

                <!-- Liên hệ -->
                <div class="col-md-4 col-lg-3 col-xl-3 mx-auto mt-3">
                    <h6 class="text-uppercase mb-4 font-weight-bold">Liên hệ</h6>
                    <p><i class="fas fa-home mr-3"></i> 123 Nguyễn Văn Cừ, Q.5, TP. HCM</p>
                    <p><i class="fas fa-envelope mr-3"></i> support@soundhub.vn</p>
                    <p><i class="fas fa-phone mr-3"></i> 0909 123 456</p>
                    <p><i class="fas fa-print mr-3"></i> (028) 1234 5678</p>
                </div>

                <!-- Mạng xã hội -->
                <div class="col-md-3 col-lg-2 col-xl-2 mx-auto mt-3">
                    <h6 class="text-uppercase mb-4 font-weight-bold">Kết nối với chúng tôi</h6>

                    <a class="btn btn-primary btn-floating m-1" style="background-color: #3b5998" href="#!" role="button">
                        <i class="fab fa-facebook-f"></i>
                    </a>
                    <a class="btn btn-primary btn-floating m-1" style="background-color: #55acee" href="#!" role="button">
                        <i class="fab fa-twitter"></i>
                    </a>
                    <a class="btn btn-primary btn-floating m-1" style="background-color: #dd4b39" href="#!" role="button">
                        <i class="fab fa-google"></i>
                    </a>
                    <a class="btn btn-primary btn-floating m-1" style="background-color: #ac2bac" href="#!" role="button">
                        <i class="fab fa-instagram"></i>
                    </a>
                    <a class="btn btn-primary btn-floating m-1" style="background-color: #0082ca" href="#!" role="button">
                        <i class="fab fa-linkedin-in"></i>
                    </a>
                    <a class="btn btn-primary btn-floating m-1" style="background-color: #333333" href="#!" role="button">
                        <i class="fab fa-github"></i>
                    </a>
                </div>
            </div>
            <!--Grid row-->
        </section>
        <!-- Section: Links -->
    </div>
    <!-- Grid container -->

    <!-- Copyright -->
    <div class="text-center p-3" style="background-color: rgba(0, 0, 0, 0.2)">
        © 2025 Bản quyền thuộc về:
        <a class="text-white" href="home">Sound Hub</a>
    </div>
    <!-- Copyright -->
</footer>
